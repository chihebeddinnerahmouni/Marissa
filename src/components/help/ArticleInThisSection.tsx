...
const ArticleInThisSection = ({category, questions}:any) => {
  ...
  return (
    <div
      className={`relative w-full py-2 border-t border-main px-2 lg:col-span-4 lg:px-0 lg:border-none ${
        !isListOpen && "border-b-1"
      }`}
    >
      <div
        ...
        >
          {questions.map((question: any, index: number) => (
            <div
              key={index}
              onClickCapture={() => {
                isMobile && setIsListOpen(false);
                navigate(`/help/${category}/${question.id}`);
              }}
              className={`menu-item p-2 rounded-lg cursor-pointer text-sm font-semibold transition duration-300 ease-in-out transform ${
                Number(questionId) === question.id
                  ? "bg-main text-white shadow-md"
                  : "bg-white text-gray-700 hover:shadow-lg hover:bg-gray-100"
              }`}
            >
              {t(question.question)}
            </div>
